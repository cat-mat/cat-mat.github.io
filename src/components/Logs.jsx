...
const Logs = () => {
  ...
  // Only declare showImportModal and setShowImportModal once
  ...
  const renderEntryData = (entry) => {
    ...
    Object.entries(entry).forEach(([key, value]) => {
      ...
      } else if (key === 'pill_pack_start_date' && value) {
        dataItems.push({
          label: 'Pill Pack Start Date',
          value: value,
          type: 'tracking'
        })
      } else if (key !== 'id' && key !== 'timestamp' && key !== 'type' && 
                 key !== 'sync_status' && key !== 'is_deleted' && key !== 'created_at' && 
                 key !== 'updated_at' && value !== undefined && value !== null && value !== '' && key !== 'pill_pack_start_date') {
        const item = TRACKING_ITEMS[key]
        if (item) {
          let displayValue = value
          if (Array.isArray(value)) {
