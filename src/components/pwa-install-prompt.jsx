...
const PWAInstallPrompt = () => {
  ...
  useEffect(() => {
    ...
    const handleAppInstalled = () => {
      ...
      addNotification({
        type: 'success',
        title: i18n.t('pwa.installed.title'),
        message: i18n.t('pwa.installed.message')
      })
    }
      ...
      <div className="meadow-card p-4 shadow-lg border-2 border-purple-200">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              <h3 className="font-semibold text-gray-900">{i18n.t('pwa.prompt.title')}</h3>
              <p className="text-sm text-gray-600">{i18n.t('pwa.prompt.subtitle')}</p>
            </div>
          </div>
          <div className="flex space-x-2">
            ...
            >
              {i18n.t('common.later', { default: 'Later' })}
            </button>
            <button
              onClick={handleInstall}
              className="btn-primary text-sm px-4 py-2"
            >
              {i18n.t('pwa.prompt.install')}
            </button>
          </div>
        </div>
      </div>
